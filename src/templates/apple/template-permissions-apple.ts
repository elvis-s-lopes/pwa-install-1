...
const template = (name: string, description: string, installDescription: string, disableDescription: boolean, icon: string, manifest: WebAppManifest, installAvailable: any, hideDialog: any, howToForApple: any, howToRequested: boolean) => {
        ...
        <aside id="pwa-install-element">
            <article class="install-dialog apple ${classMap(installDialogClassesApple())} dialog-body">
                ...
                <div class="about">
                    ...
                    <div class="description">${location.hostname}</div>
                </div>
                ${!disableDescription? html`<div class="welcome-to-install">
                    ${installDescription? installDescription: msg('Para uma melhor experiência, recomendamos que você ative as notificações no botão abaixo.')}</div>` 
                : '' }
